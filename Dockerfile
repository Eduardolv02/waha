...
# Activar Corepack y preparar Yarn 3.6.3
RUN npm install -g corepack \
    && corepack enable \
    && corepack prepare yarn@3.6.3 --activate

# Instalar git (necesario para dependencias desde repositorios)
...
# Instalar ffmpeg, libvips y utilidades
...
# Instalar fuentes y dependencias si se usa Chromium/Chrome
RUN if [ "$USE_BROWSER" = "chromium" ] || [ "$USE_BROWSER" = "chrome" ]; then \
    apt-get update && apt-get install -y \
        ...
        libasound2 libatk-bridge2.0-0 libgtk-3-0 libdrm2 ca-certificates --no-install-recommends \
    && rm -rf /var/lib/apt/lists/*; \
    fi

# Instalar Chromium o Chrome según USE_BROWSER
